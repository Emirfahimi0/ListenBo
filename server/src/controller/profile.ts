...
export const getAudiosByPlaylist: RequestHandler = async (req, res) => {
	...
	if (isValidObjectId(playlistId) === false) return res.status(422).json({ error: "Invalid playlist Id!" });
	const aggregationLogics = [
		{ $match: { _id: new Types.ObjectId(playlistId), visibility: { $ne: "Private" } } },
		{
			$project: {
				items: { $slice: ["$items", parseInt(limit) * parseInt(pageNo), parseInt(limit)] },
				title: "$title",
			},
		},
		{ $unwind: "$items" },
		{ $lookup: { from: "audios", localField: "items", foreignField: "_id", as: "audios" } },
		{ $unwind: "$audios" },
		{ $lookup: { from: "users", localField: "audios.owner", foreignField: "_id", as: "userInfo" } },
		{ $unwind: "$userInfo" },
		{
			$group: {
		...
		{ $project: { _id: 0, id: "$_id.id", title: "$_id.title", audios: "$$ROOT.audios" } },
	];
	const [result] = await playlistModel.aggregate(aggregationLogics);

	if (!result) {
		const [autoPlaylistResult] = await autoPlaylistModel.aggregate(aggregationLogics);
...
export const getPrivateAudiosByPlaylist: RequestHandler = async (req, res) => {
	const { limit = "20", pageNo = "0" } = req.query as TPaginationQuery;
	const { playlistId } = req.params;
...
export const getIsFollowing: RequestHandler = async (req, res) => {
	...
	const user = await userModel.findOne({ _id: profileId, followers: req.user.id });
