...
export const createPlaylist: RequestHandler = async (req: IReqPlaylistDocument, res) => {
	const { title, visibility, resId } = req.body;
	const ownerId = req.user.id;

	if (resId !== undefined) {
		const audio = await audioModel.findById(resId);
		if (audio === null) return res.status(404).json({ error: "Could not found the audio" });
	}
...
export const updatePlaylist: RequestHandler = async (req: IReqUpdatePlaylistDocument, res) => {
	...
	if (playlist === null) return res.status(404).json({ error: "Playlist Not Found!" });

	if (item !== undefined) {
		const audio = await audioModel.findById(item);
		if (audio === null) return res.status(404).json({ error: "Audio Not Found!" });
...
export const removePlaylist: RequestHandler = async (req, res) => {
	...
	if (isValidObjectId(playlistId) === false) return res.status(404).json({ error: "Invalid playlist id!" });
	...
	if (resId !== undefined) {
		if (isValidObjectId(resId) === false) return res.status(404).json({ error: "Invalid audio id!" });
		...
		if (updatedPlaylist === null) return res.status(404).json({ error: "Playlist not found!" });
	}
...
export const getPlaylistByProfile: RequestHandler = async (req, res) => {
	...
	if (userPlaylist === null) return res.status(404).json({ error: "No Playlists available!" });
