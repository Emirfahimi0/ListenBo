...
export const verifyEmail: RequestHandler = async (req: IVerifyEmail, res) => {
	...
	if (isVerified === false) return res.status(403).json({ error: "Invalid. Token does not match!" });
...
export const reVerifyEmail: RequestHandler = async (req, res) => {
	...
	if (user.verified === true) return res.status(422).json({ error: "Your account is already verified!" });
	...
	handleEmailSender({ name: user?.name, email: user?.email }, tokenTemplate, MAIL.RE_VERIFY_EMAIL_SUBJECT);
...
export const updatePassword: RequestHandler = async (req, res) => {
	...
	const user = await userModel.findById(userId);
	if (user === null) return res.status(403).json({ error: " unathorized access!" });
...
export const signIn: RequestHandler = async (req, res) => {
	...
	if (user === null) return res.status(403).json({ error: "Email or password mismatch!" });

	const isMatch = await user.comparePassword(password);

	if (isMatch === false) return res.status(403).json({ error: "Password is not match!" });
...
export const updateProfile: RequestHandler = async (req: IReqWithFiles, res) => {
	const { name } = req.body;
	...
	if (typeof name !== "string") return res.status(422).json({ error: "Invalid profile name" });

	if (name.trim().length < 3) return res.status(422).json({ error: "Invalid profile name, name is too short" });
