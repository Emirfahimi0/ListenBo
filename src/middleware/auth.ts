...
export const verifyPassword: RequestHandler = async (req, res, next) => {
	...
	if (result === false) return res.status(403).json({ error: "Unauthorized access! Token is not valid" });
...
export const verifyAuth: RequestHandler = async (req, res, next) => {
	...
	if (token === undefined) return res.status(403).json({ error: "Unauthorized Request!" });
	...
	if (user === null) return res.status(403).json({ error: "Unauthorized Request!" });
...
export const fileParser: RequestHandler = async (req: IReqWithFiles, res, next) => {
...
export const isVerified: RequestHandler = async (req, res, next) => {
	...
	if (isVerified === false) return res.status(403).json({ error: "Please verify your email account!" });
	next();
};
